 ...
 */
public interface FinBillingRecordService extends BaseService<FinBillingRecordResp, FinBillingRecordDetailResp, FinBillingRecordQuery, FinBillingRecordReq>, IService<FinBillingRecordDO> {
    ...
    /**
     * 根据签名 token 获取账单信息（公开接口，无需登录）
     *
     * @param token 签名链接token
     * @return 账单信息（含明细列表）
     */
    Map<String, Object> getBySignToken(String token);

    /**
     * 根据签名 token 提交签名（公开接口，无需登录）
     *
     * @param token        签名链接token
     * @param signImageUrl 签名图片Base64数据
     * @param clientIp     客户端IP
     */
    void signByToken(String token, String signImageUrl, String clientIp);
}
